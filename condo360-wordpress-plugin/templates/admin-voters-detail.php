...
<div class="voters-detail-content">
    <h3><?php echo esc_html($votersData['survey']['title'] ?? 'Sin título'); ?></h3>
    ...
    <div class="survey-info-section">
        ...
        <p><strong>Estado:</strong> <?php echo ($votersData['survey']['status'] ?? 'unknown') === 'open' ? 'Activa' : 'Cerrada'; ?></p>
        
        <h4>Fecha de Inicio y Fin</h4>
        <p>
            <strong>Inicio:</strong> <?php echo isset($votersData['survey']['start_date']) ? date_i18n(get_option('date_format'), strtotime($votersData['survey']['start_date'])) : 'No disponible'; ?><br>
            <strong>Fin:</strong> <?php echo isset($votersData['survey']['end_date']) ? date_i18n(get_option('date_format'), strtotime($votersData['survey']['end_date'])) : 'No disponible'; ?>
        </p>
    </div>
    ...
    <div class="participation-stats-section">
        ...
        <div class="stats-display">
            <div class="stat-row">
                ...
                <span class="stat-value"><?php echo esc_html($votersData['statistics']['total_eligible_voters'] ?? '0'); ?></span>
            </div>
            
            <div class="stat-row">
                <span class="stat-label">Votos Recibidos:</span>
                <span class="stat-value"><?php echo esc_html($votersData['statistics']['actual_voters'] ?? '0'); ?></span>
            </div>
            
            <div class="stat-row">
                <span class="stat-label">Porcentaje de Participación:</span>
                <span class="stat-value"><?php echo esc_html($votersData['statistics']['participation_percentage'] ?? '0'); ?>%</span>
            </div>
        </div>
        ...
        <div class="participation-bar-section">
            ...
            <div class="participation-bar-container">
                <div class="participation-bar">
                    <div class="participation-bar-fill" style="width: <?php echo esc_attr($votersData['statistics']['participation_percentage'] ?? '0'); ?>%"></div>
                </div>
                <div class="participation-percentage-text"><?php echo esc_html($votersData['statistics']['participation_percentage'] ?? '0'); ?>%</div>
            </div>
        </div>
    </div>
    
    <div class="voters-list-section">
        <h4>Lista de Votantes (<?php echo count($votersData['voters'] ?? []); ?> de <?php echo $votersData['pagination']['total_voters'] ?? '0'; ?> propietarios)</h4>
        ...
        <?php else: ?>
            ...
            <!-- Pagination -->
            <?php if (($votersData['pagination']['total_pages'] ?? 1) > 1): ?>
                <div class="voters-pagination">
                    <div class="pagination-info">
                        Mostrando <?php echo ((($votersData['pagination']['current_page'] ?? 1) - 1) * ($votersData['pagination']['per_page'] ?? 10)) + 1; ?> 
                        a <?php echo min(($votersData['pagination']['current_page'] ?? 1) * ($votersData['pagination']['per_page'] ?? 10), $votersData['pagination']['total_voters'] ?? 0); ?> 
                        de <?php echo $votersData['pagination']['total_voters'] ?? 0; ?> votantes
                    </div>
                    
                    <div class="pagination-controls">
                        <?php if (($votersData['pagination']['current_page'] ?? 1) > 1): ?>
                            <button class="pagination-btn" data-page="<?php echo ($votersData['pagination']['current_page'] ?? 1) - 1; ?>">
                                ← Anterior
                            </button>
                        <?php endif; ?>
                        ...
                        <div class="pagination-numbers">
                            <?php
                            $currentPage = $votersData['pagination']['current_page'] ?? 1;
                            $totalPages = $votersData['pagination']['total_pages'] ?? 1;
                        ...
                        <?php if (($votersData['pagination']['current_page'] ?? 1) < ($votersData['pagination']['total_pages'] ?? 1)): ?>
                            <button class="pagination-btn" data-page="<?php echo ($votersData['pagination']['current_page'] ?? 1) + 1; ?>">
                                Siguiente →
                            </button>
                        <?php endif; ?>

...
-- For existing polls, we'll use the question field as the title and create a generic description
INSERT INTO condo360_polls (id, title, description, status, start_date, end_date, created_at)
SELECT 
    id, 
    question as title, 
    'Encuesta migrada desde la versión anterior' as description, 
    status, 
    start_date, 
    end_date, 
    created_at 
...
-- Check that polls were migrated correctly
SELECT * FROM condo360_polls LIMIT 5;

-- Check that questions were migrated correctly
SELECT * FROM condo360_poll_questions LIMIT 5;

-- Check that votes were migrated correctly
SELECT * FROM condo360_votes LIMIT 5;

-- Check relationships
SELECT p.title, q.question_text, q.options 
FROM condo360_polls p
JOIN condo360_poll_questions q ON p.id = q.poll_id
LIMIT 10;

SELECT p.title, q.question_text, v.answer, v.wp_user_id
FROM condo360_polls p
JOIN condo360_poll_questions q ON p.id = q.poll_id
JOIN condo360_votes v ON q.id = v.question_id
LIMIT 10;

-- After verifying the migration was successful, you can drop the old tables:
